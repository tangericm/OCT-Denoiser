...
def test_axial_deconvolution_forward_1d_shape():
    mod = AxialDeconvolution(h_kernel=torch.ones(64, dtype=torch.complex64), lam=1.0e-3)
...
def test_spectrum_unet_1d_forward_uses_axial_deconv():
    model = create_model(
        "spectrum_unet_1d",
        base=16,
        deconv_h=torch.ones(288, dtype=torch.complex64),
