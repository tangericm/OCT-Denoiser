"""NAFNet — Nonlinear Activation Free Network (Chen et al., ECCV 2022).

State of the art on image restoration benchmarks with a deliberately minimal
block: no ReLU/GELU anywhere. Nonlinearity comes from SimpleGate, an elementwise
product of two channel halves, and channel attention is reduced to a global
average plus a 1x1 conv.

Relevant here because it is markedly cheaper per unit quality than transformer
restoration models, which matters for the edge-deployment target.
"""
...
class SimpleGate(nn.Module):
    """Split channels in half and multiply — NAFNet's only nonlinearity."""
...
class NAFBlock(nn.Module):
    def __init__(self, c: int, dw_expand: int = 2, ffn_expand: int = 2):
        ...
        self.sg = SimpleGate()
        # Simplified channel attention: global context, one 1x1, no activation.
...
class NAFNet(nn.Module):
    def __init__(self, in_ch: int = 1, out_ch: int = 1, base: int = 32,
                 enc_blocks=(1, 1, 1, 2), middle_blocks: int = 2,
                 dec_blocks=(1, 1, 1, 1)):
        super().__init__()
        ...
        for n in dec_blocks:
            # PixelShuffle upsampling: no transposed-conv checkerboard artefacts,
            # which show up badly on speckle.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for enc, down in zip(self.encoders, self.downs, strict=True):
            ...
            y = down(y)
        y = self.middle(y)
        for dec, up, skip in zip(self.decoders, self.ups, skips[::-1], strict=True):
            y = up(y)
            y = y + skip
            y = dec(y)
        out = self.ending(y)
...
def build_nafnet(*, base: int = 32, in_ch: int = 1, **_) -> nn.Module:
